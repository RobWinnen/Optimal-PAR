...
,PEARL_USER_CLASS.GROUP_CODE_DESC                                                       as Order_Blocked_By_User_Group
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW,'')                                         as Order_Block_Value
,coalesce(case
...
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD,'')                                         as Previous_Order_Block_Value
,coalesce(case
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'FAKSK' then CP_BILL_BLK_OLD.BILL_BLK_DESC
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'LIFSK' then CP_DLVRY_BLK_OLD.DLVRY_BLK_DESC
...
from 		DEV_EMEA_CDH_DB.UNIFIED.CP_SALES_ORD_HDR           						    CP_SALES_ORD_HDR --> Sales Order Header info
left join   DEV_SDH_DB.CP.VBAK                                 						    VBAK --> Sales Order Header info which is not yet available in CP_SALES_ORD_HDR
        on  VBAK.vbeln = CP_SALES_ORD_HDR.SALES_ORD_ID
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_CUST                    						    CP_CUST --> Customer Description
...
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                					        CP_BILL_BLK_OLD -->
        on  CP_BILL_BLK_OLD.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                					        CP_BILL_BLK_NEW
        on  CP_BILL_BLK_NEW.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_DLVRY_BLK                					    CP_DLVRY_BLK_OLD
        on  CP_DLVRY_BLK_OLD.DLVRY_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_DLVRY_BLK                					    CP_DLVRY_BLK_NEW
        on  CP_DLVRY_BLK_NEW.DLVRY_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW
...
,PEARL_USER_CLASS.GROUP_CODE_DESC                                                       as Order_Released_By_User_Group
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW,'')                 						as Order_Release_Value
,coalesce(case
...
where       CP_SALES_ORD_HDR.SALES_DOC_TYP_ID    IN ('KE', 'ZFKE')
    ...
    and     RPT_LOC_HRCHY.HRCHY_LVL1_DESC   = 'EMEA'
	and     RPT_LOC_HRCHY.HRCHY_LVL2_DESC   = 'Western Europe' --> Use UMD / Dropfile solution instead of hardcoded country codes / Regions
    and     (CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW = ' '     or CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW IS NULL)
    and     (CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD <> ' '    or CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD IS NOT NULL)
    order by CP_SALES_ORD_HDR.ORD_CREATE_DT,CP_SALES_ORD_HDR.SALES_ORD_ID,CP_SALES_ORD_DTL_CHG_LOG.CHG_TS,CP_SALES_ORD_DTL_CHG_LOG.CHG_BY_USER_ID
...
,PEARL_USER_CLASS.GROUP_CODE_DESC                                                       as Line_Blocked_By_User_Group
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW,'')                                         as Line_Block_Value
,coalesce(case
...
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD,'')                                         as Previous_Line_Block_Value
,coalesce(case
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'FAKSP' then bill_blk_prev.BILL_BLK_DESC
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'LIFSP' then del_blk_prev.DLVRY_BLK_DESC
...
FROM 		DEV_EMEA_CDH_DB.UNIFIED.CP_SALES_ORD_DTL                                    CP_SALES_ORD_DTL --> order details from unified
left join   DEV_SDH_DB.CP.VBAP                                                          VBAP --> raw order details
...
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_DIV                                              CP_DIV --> Division Description
        on  CP_DIV.div_id = CP_SALES_ORD_DTL.div_id 
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_CUST                                             CP_CUST --> Customer Description
        on  CP_CUST.CUST_ID = CP_SALES_ORD_DTL.SOLD_TO_CUST_ID
inner join  DEV_EMEA_CDH_DB.UNIFIED.CP_GEO_ISO                                          CP_GEO_ISO --> Country Geo info used in filter
        ON  CP_CUST.CNTRY_ID = CP_GEO_ISO.GEO_ISO_ID
left join   (SELECT
             HK_CP_ENTITY
            ,HRCHY_NM
...
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                bill_blk_prev
        on  bill_blk_prev.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                bill_blk_new
        on  bill_blk_new.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_DLVRY_BLK               del_blk_prev
        on  del_blk_prev.DLVRY_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
left join   DEV_EMEA_CDH_DB.UNIFIED.CP_DLVRY_BLK               del_blk_new
        on  del_blk_new.DLVRY_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW
left join   DEV_EMEA_RDH_DB.CUSTOMERCARE.PEARL_USER_CLASSIFICATION                      PEARL_USER_CLASS
        on  PEARL_USER_CLASS.SOURCE_CODE = CP_SALES_ORD_DTL_CHG_LOG.CHG_BY_USER_ID
...
where       CP_SALES_ORD_DTL.ITEM_CAT_ID         = 'KEN'
        ...
        --and (CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD = ' '     or  CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD IS NULL) 
        --and (CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW <> ' '    and CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW IS NOT NULL)
...
,CAST (CP_SALES_ORD_DTL_CHG_LOG.CHG_TS AS DATE)                                             as Line_Released_Date
,CAST (CP_SALES_ORD_DTL_CHG_LOG.CHG_TS AS TIME)                                             as Line_Released_Time
,CP_SALES_ORD_DTL_CHG_LOG.CHG_BY_USER_ID                                                  	as Line_Released_By_User_ID
,PEARL_USER_CLASS.GROUP_CODE_DESC                                                           as Line_Released_By_User_Group
,coalesce(CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW,'')                                             as Line_Block_Value
,coalesce(case
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'FAKSP' then bill_blk_new.BILL_BLK_DESC
    when CP_SALES_ORD_DTL_CHG_LOG.FIELD_NM = 'LIFSP' then del_blk_new.DLVRY_BLK_DESC
...
left join   (select
        ...
        on          CP_SALES_ORD_DTL_CHG_LOG.SALES_ORD_ID = CP_SALES_ORD_DTL.SALES_ORD_ID
        and COALESCE(CAST(SUBSTR(CP_SALES_ORD_DTL_CHG_LOG.TBL_KEY, 14, 6) AS INT),'000000') = CP_SALES_ORD_DTL.SALES_ORD_LN_NUM 
left join   PROD_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                bill_blk_prev
        on  bill_blk_prev.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
left join   PROD_EMEA_CDH_DB.UNIFIED.CP_BILL_BLK                bill_blk_new
        on  bill_blk_new.BILL_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_NEW
left join   PROD_EMEA_CDH_DB.UNIFIED.CP_DLVRY_BLK               del_blk_prev
        on  del_blk_prev.DLVRY_BLK_ID = CP_SALES_ORD_DTL_CHG_LOG.UNIT_OLD
