...
,CAST (VBEP.EDATU as date)                                  					AS Requested_Delivery_Date --//needs to come from a new (to be created) view or table in FOUNDATION / UNIFIED = CP_SALES_ORD_DTL_SCHED
...
,COALESCE(CAST(VBEP.BMENG as INT),'0')                      					AS Confirmed_Qty --//needs to come from a new (to be created) view or table in FOUNDATION / UNIFIED = CP_SALES_ORD_DTL_SCHED
...
left join  (select VBELN, POSNR, max(EDATU) as EDATU, sum(BMENG) as BMENG
             from DEV_SDH_DB.CP.VBEP
             group by vbeln, posnr)                           				VBEP --> Schedueled Line Info //NEEDS A VIEW IN FOUNDATION: CP_SALES_ORD_DTL_SCHED and needs HK_CP_SALES_ORD_DTL_SCHED (vbep.vbeln + vbep.posnr)
        on  CP_SALES_ORD_DTL.SALES_ORD_ID = VBEP.VBELN						--//can be removed once CP_SALES_ORD_DTL_SCHED is in place
        and CP_SALES_ORD_DTL.SALES_ORD_LN_NUM = vbep.posnr					--//needs to be replaced by HK_CP_SALES_ORD_DTL_SCHED once CP_SALES_ORD_DTL_SCHED is in place
