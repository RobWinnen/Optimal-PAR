...
,COALESCE(del_dtl.GTIN_ID,CFN_id)                           AS GTIN
,COALESCE(matl_char.version_number,'')                      AS Version_Number
,ord_dtl.MATL_ID                                            AS Material
,CAST(ord_dtl.ORD_QTY as INT)                               AS Order_Qty
,COALESCE(CAST(vbep.BMENG as INT),'0')                      AS Confirmed_Qty_vbep
,COALESCE(CAST(del_dtl.DLVRY_DTL_QTY as INT),'0')           AS Confirmed_Qty
,ord_dtl.BASE_UOM_ID                                        AS UOM
,COALESCE(ord_dtl.ln_dlvry_blk_id,'')                       AS Delivery_Block
,COALESCE(ord_dtl.reject_reasn_id,'')                       AS Current_Rejection_Reason
,COALESCE(cd.value_old,'')                                  AS Previous_Rejection_Reason
,COALESCE(cd.value_new,'')                                  AS Updated_Rejection_Reason
,cd.UDATE                                                   AS Rejection_Update_Date
,cd.UTIME                                                   AS Rejection_Update_Time
,COALESCE(cd.USERNAME,'')                                   AS Rejection_Updated_By
,CASE
    WHEN cd.value_old = '97' AND cd.value_new = ' '              THEN 'Released'
    WHEN (cd.value_new = '97' or ord_dtl.reject_reasn_id = '97') THEN 'Rejected'
    ELSE 'Different Rejection Reason'
END                                                         AS Released_Check
,VBAP.ZOTC_SREPCODE                                         AS Sales_Rep_vbap
,sales_rep_vbap.CUST_NM                                     AS Sales_Rep_Name_vbap
,vbap.zotc_level3                                           AS Region_vbap
,vbap.zotc_level4                                           AS District_vbap
,vbap.zotc_tertry                                           AS Territory_vbap
,vbap.zotc_sforce                                           AS Sales_Force_Description_VBAP
,xref.CUST_ID_SALES_REP                                     AS Sales_Rep
,COALESCE(sales_rep.CUST_NM,'')                             AS Sales_Rep_Name
...
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_ORD_HDR           ord_hdr--
        on  ord_hdr.HK_CP_SALES_ORD_HDR = ord_dtl.HK_CP_SALES_ORD_HDR
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_CUST                    cust--
...
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_MATL                    matl--
...
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_MPG                     mpg--
        on  mpg.mpg_id = ord_dtl.mpg_id 
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_ORG               org--
...
left join  PROD_EMEA_CDH_DB.UNIFIED.CP_EQUIP_SALES_ACCT_XREF   xref--
...
left join   PROD_SDH_DB.CP.VBEP                                 vbep--
        on  ord_dtl.SALES_ORD_ID = vbep.vbeln
        and ord_dtl.SALES_ORD_LN_NUM = vbep.posnr
left join   PROD_SDH_DB.CP.VBAP                                 vbap--
        on  ord_dtl.SALES_ORD_ID = vbap.vbeln
        and ord_dtl.SALES_ORD_LN_NUM = vbap.posnr
left join   PROD_EMEA_CDH_DB.UNIFIED.CP_CUST                    sales_rep_vbap--
        on  sales_rep_vbap.CUST_ID = VBAP.ZOTC_SREPCODE 
left join   PROD_EMEA_CDH_DB.UNIFIED.CP_CUST                    sales_rep--
...
left join   (SELECT
             ...
             WHERE TABNAME IN ('VBAP', 'VBAK', 'VBEP')
             AND FNAME IN ('ABGRU')
             AND (cdpos.value_old = '97' or cdpos.value_new = '97')) cd
        on  cd.objectid = ord_hdr.SALES_ORD_ID
where   ord_hdr.sales_doc_typ_id = 'KB'
and     ord_hdr.sales_ord_id IN ('6213561441','6213573130','6213387559','6213585607','6213461464')
and     (cd.value_old = '97' or cd.value_new = '97' or ord_dtl.reject_reasn_id = '97')
and     ord_hdr.ORD_CREATE_DT > '2022-01-01'

--select
--acct.CUST_ID_SALES_REP
--from        PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_STRUCT_HRCHY hrchy
--inner join  PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_ORD_HDR ord_hdr
--        on  ord_hdr.SALES_ORG_ID        = hrchy.SALES_ORG_ID
--        and ord_hdr.DIST_CHAN_ID        = hrchy.DIST_CHAN_ID
--inner join  PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_ORD_DTL ord_dtl
--        on  ord_dtl.HK_CP_SALES_ORD_HDR = ord_hdr.HK_CP_SALES_ORD_HDR
--        and ord_dtl.mpg_id              = hrchy.MPG_ID
--inner join  PROD_EMEA_CDH_DB.UNIFIED.CP_SALES_ACCT acct
--        on  acct.DIST_CHAN_ID           = hrchy.DIST_CHAN_ID
--        and acct.HK_CP_SALES_STRUCT     = hrchy.HK_CP_SALES_STRUCT 
--        and acct.HK_CP_SFORCE           = hrchy.HK_CP_SFORCE
--        and acct.HK_CP_MPG              = hrchy.HK_CP_MPG
--        and acct.SALES_ACCT_STAT_CD = 'Active'
--        and acct.PRIM_REP_FLAG = 'Y'